...
test('lets an expired listener sign in and continue generation without losing the request', async ({ page }) => {
  ...
  await page.route('**/api/podcasts/sessions', async (route) => {
    ...
    await route.fulfill(
      generationAttempts === 1
        ...
        : { status: 201, contentType: 'application/json', body: JSON.stringify({ session }) },
    );
  });
  await page.route('**/api/auth/login', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ message: 'Login successful' }) });
  });
  ...
  await expect(page.getByRole('heading', { name: 'Sign in to generate' })).toBeVisible();
